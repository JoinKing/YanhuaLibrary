# 框架介绍
**该MVVM框架是以谷歌DataBinding+LiveData+ViewModel为基础，整合Okhttp+RxJava+Retrofit+Glide等流行模块，加上各种原生控件自定义的BindingAdapter，以及Rxbus通讯组件，让组件、线程间通讯更加简洁、让事件与数据源完美绑定**。从此不再**findViewById()**、**setText()**、**setOnClickListener()**
...
- **流行框架**
	...
	1.  [retrofit](https://github.com/square/retrofit)+[okhttp](https://github.com/square/okhttp)+[rxJava](https://github.com/ReactiveX/RxJava) **负责网络请求工作**；
	
	2.  [gson](https://github.com/google/gson) **负责解析json数据**；
	
	3.  [glide](https://github.com/bumptech/glide) **负责加载图片**；
	
	4.  [rxlifecycle](https://github.com/trello/RxLifecycle) **负责管理view的生命周期；与网络请求共存亡**；
	
	5.  [rxbinding](https://github.com/JakeWharton/RxBinding) **结合databinding扩展UI事件**；
	
	6.  [rxpermissions](https://github.com/tbruyelle/RxPermissions) **负责Android 6.0权限申请**；
	
	7.  [material-dialogs](https://github.com/afollestad/material-dialogs) **一个漂亮的、流畅的、可定制的material design风格的对话框**。

- **数据绑定**

	满足google目前控件支持的databinding双向绑定，并扩展原控件一些不支持的数据绑定。
	
	**例如：**	

	将图片的url路径绑定到ImageView控件中，在BindingAdapter方法里面则使用Glide加载图片；View的OnClick事件在BindingAdapter中方法使用RxView防重复点击，再把事件回调到ViewModel层，实现xml与ViewModel之间数据和事件的绑定(框架里面部分扩展控件和回调命令使用的是@kelin原创的)。

- **基类封装**

	1.  专门针对MVVM模式打造的BaseActivity、BaseFragment、BaseViewModel；
	
	2.  在View层中不再需要定义ViewDataBinding和ViewModel，直接在BaseActivity、BaseFragment上限定泛型即可使用。
	
	3.  普通界面只需要编写Fragment，然后使用ContainerActivity盛装(代理)，这样就不需要每个界面都在AndroidManifest中注册一遍。
...
- **全局操作**
	...
	8. 集成腾讯的TBS浏览服务，更高的提升VebView的兼容性，H5项目可提升30%速度。
...
dependencies {
    ...
    implementation 'com.github.king:mvvm:1.0.15'
}
```
...
继承**mvvm**中的BaseApplication，或者调用
...
<layout>
    <data>
        <variable
            type="com.king.mvvm.ui.login.LoginViewModel"
            name="viewModel"
        />
    </data>
    .....
...
```xml
app:placeholderRes="@{R.mipmap.ic_launcher_round}"
```
> R文件需要在data标签中导入使用，如：`<import type="com.king.mvvm.R" />`
...
> RecyclerView也是很常用的一种控件，传统的方式需要针对各种业务要写各种Adapter，如果你使用了mvvm，则可大大简化这种工作量，从此告别setAdapter()。
...
> `<import type="com.xxx.bindingcollectionadapter2.LayoutManagers"
> />`</br> `<import
> type="com.github.king.binding.viewadapter.recyclerview.LineManagers"
> />`</br> `<import type="android.support.v7.widget.LinearLayoutManager"
> />`
...
square出品的框架，用起来确实非常方便。**MVVM**中引入了
```gradle
...
//缓存存放的文件
File httpCacheDirectory = new File(mContext.getCacheDir(), "xxx");
//缓存对象
...
```
#### 2.3.3、Cookie管理
**MVVM**提供两种CookieStore：**PersistentCookieStore** (SharedPreferences管理)和**MemoryCookieStore** (内存管理)，可以根据自己的业务需求，在构建okhttp时加入相应的cookieJar
```java
OkHttpClient okHttpClient = new OkHttpClient.Builder()
...
**注意：** 由于BaseActivity/BaseFragment都实现了LifecycleProvider接口，并且默认注入到ViewModel中，所以在调用请求方法时可以直接调用getLifecycleProvider()拿到生命周期接口。如果你没有使用 **mvvm** 里面的BaseActivity或BaseFragment，使用自己定义的Base，那么需要让你自己的Activity继承RxAppCompatActivity、Fragment继承RxFragment才能用`RxUtils.bindToLifecycle(lifecycle)`方法。
#### 2.3.5、网络异常处理
...
> mvvm中自定义了一个[ExceptionHandle]，已为你完成了大部分网络异常的判断，也可自行根据项目的具体需求调整逻辑。<br>
...
## 3、辅助功能
> 一个完整的快速开发框架，当然也少不了常用的辅助类。下面来介绍一下**MVVM**中有哪些辅助功能。
### 3.1、事件总线
> 事件总线存在的优点想必大家都很清楚了，android自带的广播机制对于组件间的通信而言，使用非常繁琐，通信组件彼此之间的订阅和发布的耦合也比较严重，特别是对于事件的定义，广播机制局限于序列化的类（通过Intent传递），不够灵活。
#### 3.3.1、RxBus
RxBus并不是一个库，而是一种模式。相信大多数开发者都使用过EventBus，对RxBus也是很熟悉。由于**MVVM**中已经加入RxJava，所以采用了RxBus代替EventBus作为事件总线通信，以减少库的依赖。
...
### 3.2、文件下载
文件下载几乎是每个app必备的功能，图文的下载，软件的升级等都要用到，mvvm使用Retrofit+Okhttp+RxJava+RxBus实现一行代码监听带进度的文件下载。
